<template>
	<div>
		...
		<p class="mb-1 mt-1">Condition :</p>
		<input ref="condition" type="text" class="form-control form-control-sm" :title="condition" v-model="condition" df-condition/>

		<p class="node-type">Condition</p>
	</div>
</template>
...
export default class Condition extends Vue {
	...
	@Ref("condition") conditionElement! : HTMLInputElement;
	...
	mounted()
	{
		this.$nextTick(() => {
			...
			this.condition = this.conditionElement.value;
		});
	}
